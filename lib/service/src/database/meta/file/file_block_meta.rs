...
impl SimpleFileBlockMetaManager {
    ...
    pub(crate) async fn modified_blocks(&self, _before: i64) -> Vec<FileBlockMeta> {
        ...
        return batis.query_decode("select * from file_block_meta where (part_hash <> cloud_file_hash or cloud_file_hash is null ) and deleted = 0 order by update_time,id",vec![]).await.unwrap();
    }
